...
async function recommendProgramFromInterest(programString: string, interestString: string) {
	...
	const completion = await openai.chat.completions.create({
		messages: [{ role: "system", content: "You are a student counselor." }, questionToAi],
		model: "gpt-3.5-turbo",
	});
...
export async function filterTheResultsFromAi(programIDString: string[], interestString: string) {
	const programs = await fetchAllProgramsJson();
	...
	// Make the HTTP request to AI and save the results
	const completion = await openai.chat.completions.create({
		messages: [{ role: "system", content: "You are a student counselor. Your answers should be formated exactly in this way [programId: number, reason: motivation] where number is the number motivation is your reasoning." }, questionToAi],
		model: "gpt-3.5-turbo",
	});
